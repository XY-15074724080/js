...
//		document.write(datatemplate(template,data));
	...
	*/
	...
	function uncamelize(s,sep){
		sep = sep || '-' ;//sep：分隔符 	判断有没有规定分隔符，没有就使用默认的
	...
	function setStyleById(element,styles){
		...
		for(property in styles){
			...
			if(element.style.setProperty){//      setProperty("background-color" )
				...
				element.style[camelize(property)]=styles[property];//->增强了行内样式，缺点：文档混乱，缺乏可重用性
			}
	...
	*/
	...
	function getStyleSheets(url,media){//此方法只适用于验证<link>引入的部分
		...
		for(var i=0;i<document.styleSheets.length;i++){
			...
			if(media){//如果有传进来media
				media=media.replace(/,\s*/,',');//把它里面的 ",(0到多个空格)" 替换为 ","   因为要考虑到media="screen,  print"情况
				//此时做的是规范化media字符串
				var sheetMedia;
				//之后做的就是规范文档下的media属性值，以便之后用来与 用来检测的media 相比较
				if(document.styleSheets[i].media.mediaText){//DOM方法
					sheetMedia=document.styleSheets[i].media.mediaText.replace(/,\s*/,',');//获取及规范media
					//Safari会添加额外的逗号和空格，所以在有空格加逗号结尾的时候要替换掉
					sheetMedia=sheetMedia.replace(/,\s*$/,'');
				}else{//ie方法
					sheetMedia=document.styleSheets[i].media.replace(/,\s*/,',');//获取及规范media
				}
				//如果media不匹配，则跳过本次循环
				if(media!=sheetMedia){
					continue;
